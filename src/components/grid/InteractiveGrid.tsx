...
import { getDistance2d, absMax, bitwiseRound } from "./utils";
...
function useCanvas(draw: Function): RefObject<HTMLCanvasElement | null> {
...
export function InteractiveGrid({
  ...
  const addGridWave = useCallback(
    throttle(
      100,
      (evt: { clientX: number; clientY: number }, isWeak: boolean): void => {
  ...
  function onPointerMove({
    clientX,
    clientY,
  }: React.PointerEvent<HTMLCanvasElement>): void {
  ...
  function onPointerUp({
    ...
    const isClick =
      Date.now() - lastPointerDownTime.current <= 150 ||
      !didPointerMoveWhileDown.current;
    if (isClick) {
