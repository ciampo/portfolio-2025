...
import { getAngleBetweenPoints, getDistance2d } from "./utils";
import type {
  GridPoint,
  GridPointWavesInfo,
  GridWave,
  GridWaveConstructorOptions,
} from "./types";
...
export function createGridWave(opts: GridWaveConstructorOptions): GridWave {
  return {
    ...
    crestAOE: opts.isWeak
      ? gridConfig.waveCrestDecay / 6
      : gridConfig.waveCrestDecay,
    strength: opts.isWeak
      ? gridConfig.waveStrengthWeak
      : gridConfig.waveStrengthStrong,
...
export function getWaveEasedCrestValue(wave: GridWave): number {
  ...
  return (
    gridConfig.waveCrestEasingFunction(wave.crestRadius / wave.easingRadius) *
    wave.maxRadius
  );
}
...
export function createGridPoints(dimensions: {
  width: number;
  height: number;
}): GridPoint[] {
  const toReturn = [];
  ...
  for (let colI = 0; colI < numCols; colI += 1) {
    for (let rowI = 0; rowI < numRows; rowI += 1) {
      const x = gridConfig.getDotCoordinate(
        (colI + 1) * gridConfig.tileSize,
        gridConfig.dotSize
      );
      const y = gridConfig.getDotCoordinate(
        (rowI + 1) * gridConfig.tileSize,
        gridConfig.dotSize
      );
      toReturn.push({
        originX: x,
...
export function updateGridPoints(
  ...
  return points.map((point) => {
    ...
    waves.forEach((wave) => {
      const { distance: distFromWave, angle: angleFromWave } =
        computePointToWaveInfo(point, wave);
      ...
      if (distFromCrest <= wave.crestAOE) {
        displayDiffFactor =
          ...
          gridConfig.wavePercEasingFunction(
            (wave.crestAOE - distFromCrest) / wave.crestAOE
          ) *
          wave.crestAOE *
          wave.strength;
